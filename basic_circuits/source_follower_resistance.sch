...
S {}
E {}
T {Basic Source follower Resistive amplifier stage.
As it can be observed the gain is smaller then 1.
But it has an smaller output equivalent impedance, this means an if a load requires some current
the vout drop in will be a lot smaller and vout will be close to its ideal value.
Generally the output impedance of an almplifier must be the smallest possible
The gain equation(level 1) dictates that the output gain is directly dependent of gm, and diretly dependent on Rs.
input NMOS stays in saturation when VGS > VTH
...
C {devices/res.sym} 430 -160 0 0 {name=R1
value=50k
footprint=1206
device=resistor
m=1}
...
C {devices/res.sym} 510 320 0 0 {name=R4
value=50k
footprint=1206
device=resistor
m=1}
